"""
AuditArc 审迹 - AI审计风险识别系统（单文件版，适用于 Streamlit Cloud）
"""
...
def generate_financial_data():
    income_statement = pd.DataFrame({
        "科目": ["营业收入","营业成本","毛利","销售费用","管理费用",
                 ...
                 "利润总额","所得税","净利润"],
        "2021年": [182450,128600,53850,9800,12300,5600,3200,22950,1200,800,23350,3500,19850],
        "2022年": [196800,141200,55600,10500,13100,6200,3800,22000,900,600,22300,3350,18950],
        "2023年": [265800,198600,67200,15200,16800,7100,6500,21600,500,1200,20900,3135,17765],
    })
    balance_sheet = pd.DataFrame({
        "科目": ["货币资金","应收账款","预付账款","存货","其他流动资产",
                 "流动资产合计","固定资产","在建工程","无形资产","长期股权投资",
                 "非流动资产合计","资产总计",
                 "短期借款","应付账款","预收账款","其他流动负债",
                 "流动负债合计","长期借款","其他非流动负债",
                 "负债合计","股本","资本公积","未分配利润","所有者权益合计"],
        "2021年": [28600,31200,5600,42800,3200,111400,89600,12300,8900,5600,
                   116400,227800,18000,22500,3200,8900,52600,35000,5800,
                   93400,50000,42000,42400,134400],
        "2022年": [24200,38900,6800,51200,4100,125200,95800,18600,9200,5600,
                   129200,254400,22000,26800,2900,10200,61900,42000,6100,
                   110000,50000,42000,62400,154400],
        "2023年": [18500,73500,9200,82600,5800,189600,108200,31500,9500,5600,
                   154800,344400,38500,31200,2100,15800,87600,68000,8600,
                   164200,50000,42000,88200,180200],
    })
    cash_flow = pd.DataFrame({
        "科目": ["销售商品收到的现金","购买商品支付的现金","支付给职工的现金",
                 "支付的各项税费","经营活动产生的现金流量净额",
                 "购建固定资产支付的现金","投资活动产生的现金流量净额",
                 "取得借款收到的现金","偿还债务支付的现金","支付股利或利润",
                 "筹资活动产生的现金流量净额","现金及现金等价物净增加额"],
        "2021年": [195600,-132800,-28600,-7800,26400,-18600,-21200,25000,-18000,-5000,2000,7200],
        "2022年": [208500,-145600,-31200,-8200,23500,-24800,-28100,32000,-22000,-6000,4000,-600],
        "2023年": [228600,-196800,-38600,-9500,4200,-35800,-39600,58500,-45000,-5000,8500,-26900],
    })
...
def parse_financial_data(inc_df, bal_df, cf_df):
    ...
    def g(df, item, cols, idx=-1):
        row = df[df["科目"] == item]
        return float(row[cols[idx]].values[0]) if not row.empty else 0.0
    rev_cur  = g(inc_df, "营业收入", inc_cols)
    rev_prev = g(inc_df, "营业收入", inc_cols, -2)
    cost_cur = g(inc_df, "营业成本", inc_cols)
    net_profit = g(inc_df, "净利润", inc_cols)
    ar_cur   = g(bal_df, "应收账款", bal_cols)
    ar_prev  = g(bal_df, "应收账款", bal_cols, -2)
    inv_cur  = g(bal_df, "存货", bal_cols)
    inv_prev = g(bal_df, "存货", bal_cols, -2)
    total_assets = g(bal_df, "资产总计", bal_cols)
    total_liab   = g(bal_df, "负债合计", bal_cols)
    op_cf = g(cf_df, "经营活动产生的现金流量净额", cf_cols)
...
def run_analysis(inc_df, bal_df, cf_df):
    ...
    cf_np = metrics["经营现金流_净利润比"]
    if rev_growth > 25 and cf_np < 0.5:
        candidates.append({"风险类型":"收入异常","触发规则":f"营收增长{rev_growth:.1f}%，但经营现金流/净利润仅{cf_np:.2f}","指标数据":{"营收增长率":f"{rev_growth:.1f}%（行业均值：11.2%）","经营现金流/净利润":f"{cf_np:.2f}（行业均值：0.92）"},"SHAP权重":{"营收增长率":42,"现金流背离":38,"应收账款联动":20},"初始评分":85})
    ar_turn = metrics["应收账款周转率"]
    ar_dev = (INDUSTRY_BENCHMARKS["应收账款周转率"] - ar_turn) / INDUSTRY_BENCHMARKS["应收账款周转率"] * 100
    if ar_dev > 25:
        candidates.append({"风险类型":"应收账款异常","触发规则":f"应收账款周转率{ar_turn:.1f}次，低于行业均值{ar_dev:.0f}%","指标数据":{"应收账款周转率":f"{ar_turn:.1f}次（行业均值：7.2次）","应收账款增长率":f"{metrics['应收账款增长率']:.1f}%（营收增长：{rev_growth:.1f}%）","应收账款/营收":f"{metrics['应收账款_营收比']:.1f}%（行业均值：15.3%）"},"SHAP权重":{"周转率偏离":35,"应收增速超营收增速":31,"账龄结构":34},"初始评分":78})
    inv_turn = metrics["存货周转率"]
    inv_dev = (INDUSTRY_BENCHMARKS["存货周转率"] - inv_turn) / INDUSTRY_BENCHMARKS["存货周转率"] * 100
    if inv_dev > 25:
        candidates.append({"风险类型":"存货异常","触发规则":f"存货周转率{inv_turn:.1f}次，低于行业均值{inv_dev:.0f}%","指标数据":{"存货周转率":f"{inv_turn:.1f}次（行业均值：4.1次）","存货规模（万元）":f"{metrics['存货（年末）']:,.0f}"},"SHAP权重":{"周转率偏离":40,"存货绝对规模":30,"跌价风险":30},"初始评分":65})
    if cf_np < 0.3:
        candidates.append({"风险类型":"现金流异常","触发规则":f"经营活动现金流/净利润={cf_np:.2f}，严重偏低","指标数据":{"经营现金流/净利润":f"{cf_np:.2f}（行业均值：0.92）","经营活动现金流（万元）":f"{metrics['经营活动现金流']:,.0f}","净利润（万元）":f"{metrics['净利润']:,.0f}"},"SHAP权重":{"现金流绝对值":45,"利润现金含量":35,"资金缺口":20},"初始评分":72})
    lev = metrics["资产负债率"]
    if lev > INDUSTRY_BENCHMARKS["资产负债率"] * 1.2:
        candidates.append({"风险类型":"负债率异常","触发规则":f"资产负债率{lev:.1f}%，高于行业均值{lev-INDUSTRY_BENCHMARKS['资产负债率']:.1f}个百分点","指标数据":{"资产负债率":f"{lev:.1f}%（行业均值：38.5%）","负债合计（万元）":f"{metrics['负债合计']:,.0f}"},"SHAP权重":{"负债率绝对值":50,"偿债能力":30,"再融资风险":20},"初始评分":55})
    # 评分 + 等级
    for c in candidates:
        score = c["初始评分"]
        if c["风险类型"] == "收入异常":
    ...
    if "收入异常" in types and "应收账款异常" in types and "现金流异常" in types:
        for c in candidates:
            if c["风险类型"] == "收入异常":
                ...
                c["风险等级"] = "极高"
                c["交叉验证"] = "✅ 收入、应收账款、现金流三维度同步异常，相互印证，确信度极高"
    ...
    radar = {}
    rv = 0
    if metrics["营收增长率"] > 25: rv += 40
    if metrics["经营现金流_净利润比"] < 0.5: rv += 40
    if metrics["应收账款增长率"] > metrics["营收增长率"]: rv += 20
    radar["收入质量"] = min(rv, 100)
    ar_b = INDUSTRY_BENCHMARKS["应收账款周转率"]
    radar["应收账款"] = min(int(max(0,(ar_b-metrics["应收账款周转率"])/ar_b)*120 + metrics["应收账款_营收比"]*1.5), 100)
    inv_b = INDUSTRY_BENCHMARKS["存货周转率"]
    radar["存货质量"] = min(int(max(0,(inv_b-metrics["存货周转率"])/inv_b)*110), 100)
    radar["现金流量"] = min(int(max(0, INDUSTRY_BENCHMARKS["经营现金流_净利润比"]-metrics["经营现金流_净利润比"])*80), 100)
    lev_b = INDUSTRY_BENCHMARKS["资产负债率"]
    radar["偿债能力"] = min(int(max(0,(metrics["资产负债率"]-lev_b)/lev_b)*100), 100)
    cnt = {k: sum(1 for c in candidates if c["风险等级"]==k) for k in ["极高","高","中","低"]}
    return {"metrics": metrics, "risks": candidates, "radar_scores": radar,
            "overall_risk": candidates[0]["风险等级"] if candidates else "低", "risk_count": cnt}
...
def generate_pdf(result):
    try:
        ...
        TS = lambda name, **kw: ParagraphStyle(name, fontName=font, **kw)
        story = []
        now = datetime.datetime.now().strftime("%Y年%m月%d日")
        story += [Spacer(1,0.3*cm),
                  Paragraph("AuditArc 审迹  |  AI审计风险预警智能体", TS("b",fontSize=11,alignment=TA_CENTER,textColor=colors.HexColor("#0066CC"),spaceAfter=8)),
                  HRFlowable(width="100%",thickness=2,color=colors.HexColor("#0066CC")),
                  Spacer(1,0.3*cm),
                  Paragraph("审计重点提示函", TS("t",fontSize=18,leading=26,alignment=TA_CENTER,textColor=colors.HexColor("#1A1A2E"),spaceAfter=4)),
                  Spacer(1,0.3*cm),
                  HRFlowable(width="100%",thickness=0.5,color=colors.HexColor("#DDD")),
                  Spacer(1,0.4*cm)]
        info = Table([["被审计单位","星锰精密机械股份有限公司（脱敏）","报告日期",now],
                      ["分析年度","2023年度","总体风险等级",result["overall_risk"]],
                      ["识别风险项","%d 项"%len(result["risks"]),"生成方式","AI自动生成"]],
        ...
        story.append(Table([[Paragraph("⚠️ 本报告由AI自动生成，仅供审计师参考，不构成最终审计意见。所有结论须经注册会计师复核确认。",
                                        TS("d",fontSize=9,textColor=colors.HexColor("#8B4513")))]],colWidths=[17*cm]))
        story[-1].setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1),colors.HexColor("#FFF8E1")),
        ...
        for i, risk in enumerate(result["risks"],1):
            ...
            t = Table([[Paragraph(f"风险{i}：{risk['风险类型']}",TS("rt",fontSize=11,textColor=colors.white)),
                        Paragraph(f"【{risk['风险等级']}风险】{risk['最终评分']}分",TS("rs",fontSize=10,textColor=colors.white,alignment=TA_RIGHT))]],
                      colWidths=[11*cm,6*cm])
            t.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1),lc),("TOPPADDING",(0,0),(-1,-1),7),
                                    ("BOTTOMPADDING",(0,0),(-1,-1),7),("LEFTPADDING",(0,0),(0,-1),10)]))
            story += [Spacer(1,0.3*cm), t]
            rows = [["触发规则", risk.get("触发规则","")]] + [[k,v] for k,v in risk.get("指标数据",{}).items()] + \
                   [["业务合理性", risk.get("业务合理性判断","")],
                    ["交叉验证", risk.get("交叉验证","")],
                    ["准则依据", risk.get("准则依据","")]]
            dt = Table([[Paragraph(r[0],TS("dk",fontSize=9,textColor=colors.HexColor("#555"))),
                         Paragraph(r[1],TS("db",fontSize=9,textColor=colors.HexColor("#333")))] for r in rows],
                       colWidths=[3.5*cm,13.5*cm])
            dt.setStyle(TableStyle([("FONTNAME",(0,0),(-1,-1),font),("GRID",(0,0),(-1,-1),0.3,colors.HexColor("#E0E0E0")),
            ...
            if procs:
                story.append(Paragraph("建议审计程序：",TS("ph",fontSize=9,textColor=colors.HexColor("#0066CC"),spaceBefore=6)))
                for j,p in enumerate(procs,1):
                    story.append(Paragraph(f"  {j}. {p}",TS("pi",fontSize=9,textColor=colors.HexColor("#666"))))
        story += [Spacer(1,0.8*cm), HRFlowable(width="100%",thickness=0.5,color=colors.HexColor("#DDD")),
                  Paragraph("本提示函由 AuditArc 审迹 AI 系统自动生成 · "+now+" · 需审计师复核确认后方可归档",
                             TS("foot",fontSize=9,textColor=colors.HexColor("#999"),alignment=TA_CENTER))]
        doc.build(story)
        buf.seek(0)
        return buf.getvalue()
...
# Session state
for k, v in [("done",False),("result",None),("inc",None),("bal",None),("cf",None),("pdf",None)]:
...
with st.sidebar:
    ...
    st.markdown("**📋 演示流程**")
    for step, done in [("Step 1  数据导入", st.session_state.inc is not None),
    ...
    if st.button("🔄 重置演示", use_container_width=True):
        for k in ["done","result","inc","bal","cf","pdf"]: st.session_state[k] = None if k!="done" else False
        st.rerun()
...
with c1:
    with st.expander("📁 上传自定义CSV文件", expanded=False):
        ui = st.file_uploader("利润表 CSV", type=["csv"], key="u1")
        ub = st.file_uploader("资产负债表 CSV", type=["csv"], key="u2")
        uc = st.file_uploader("现金流量表 CSV", type=["csv"], key="u3")
        if ui and ub and uc and st.button("✅ 加载上传数据"):
            st.session_state.inc = pd.read_csv(ui)
            st.session_state.bal = pd.read_csv(ub)
            st.session_state.cf  = pd.read_csv(uc)
            st.session_state.done = False
            st.rerun()
with c2:
    st.markdown('<div style="background:#F0F7FF;border:1px solid #CCE0FF;border-radius:10px;padding:16px"><div style="font-weight:600;color:#0066CC;margin-bottom:8px">🏭 演示数据</div><div style="font-size:12px;color:#555">星锰精密机械股份有限公司<br><span style="color:#999">（虚构·已脱敏）</span><br><br>📅 2021-2023年三年数据<br>⚠️ 含预置风险特征</div></div>', unsafe_allow_html=True)
    st.markdown("")
    if st.button("📊 加载演示数据", use_container_width=True, type="primary"):
        inc, bal, cf = generate_financial_data()
...
if st.session_state.inc is not None:
    ...
    total = len(st.session_state.inc)+len(st.session_state.bal)+len(st.session_state.cf)
    qa,qb,qc = st.columns(3)
    with qa: st.markdown(f'<div class="metric-card"><div class="metric-val" style="color:#0066CC;font-size:48px">100</div><div class="metric-label">数据质量得分</div></div>', unsafe_allow_html=True)
    with qb: st.markdown(f'<div class="metric-card"><div class="metric-val">{total}</div><div class="metric-label">数据项总数</div></div>', unsafe_allow_html=True)
    with qc: st.markdown(f'<div class="metric-card"><div class="metric-val">3</div><div class="metric-label">分析年度</div></div>', unsafe_allow_html=True)
...
if st.session_state.inc is None:
    ...
    if scan and not st.session_state.done:
        log_ph = st.empty(); bar = st.progress(0); logs = []
        steps = [("🔗 通过 MCP 协议连接数据源...",10),("📥 数据清洗与格式校验...",20),
                 ("⚙️ 规则引擎扫描：应收账款阈值校验...",35),("⚙️ 规则引擎扫描：收入-现金流勾稽核查...",50),
                 ("🤖 AI评分模型运行中（XGBoost）...",65),("🔍 业务合理性核查：季节性因素排除...",78),
                 ("🔗 多维度交叉验证（财务+经营数据）...",88),("📊 SHAP特征归因计算...",94),("📈 风险图谱生成完毕",100)]
        for msg, pct in steps:
            logs.append(msg); bar.progress(pct)
            log_ph.markdown(f'<div style="background:#0A0E2A;border-radius:8px;padding:14px;min-height:120px">'+"".join(f'<div class="log-line">▶ {l}</div>' for l in logs[-8:])+'</div>', unsafe_allow_html=True)
            time.sleep(0.55)
...
if not st.session_state.done:
    ...
    risks = res["risks"]; radar = res["radar_scores"]; cnt = res["risk_count"]
    ...
    def gs(df, item):
        ...
        row = df[df["科目"]==item]
        return [float(row[c].values[0]) for c in cols] if not row.empty else [0]*len(cols)
    with ch1:
        f1 = go.Figure()
        ...
        if st.button("📄 生成 PDF 报告", use_container_width=True, type="primary"):
            with st.spinner("正在生成审计重点提示函..."):
                ...
                if pdf:
                    ...
                    st.error("PDF生成失败，请检查依赖")
    with e2:
        if st.session_state.pdf:
            st.download_button("⬇️ 下载审计重点提示函.pdf", data=st.session_state.pdf,
                               file_name="AuditArc_审计重点提示函.pdf", mime="application/pdf", use_container_width=True)

st.markdown('<div class="disclaimer-bar">⚠️ 本系统由 AuditArc AI 智能体驱动，所有风险结论均需注册会计师专业判断复核确认。AI仅提供风险线索与程序建议，最终审计定性与报告出具权始终保留在审计师手中。本演示版本使用脱敏虚构数据，不代表任何真实公司。</div>', unsafe_allow_html=True)
