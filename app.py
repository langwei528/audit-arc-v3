...
def get_csmar_data(ticker: str, wrds_username: str, wrds_password: str, years: int = 5):
    """
    从 WRDS CSMAR 拉取A股上市公司三张报表
    ticker: 6位股票代码，如 '000001'
    返回：(income_df, balance_df, cashflow_df, company_name) 或抛出异常
    """
    import wrds
    # 连接 WRDS
    ...
    end_year = datetime.datetime.now().year - 1  # 最新完整年度
    start_year = end_year - years + 1
    ticker_6 = ticker.zfill(6)

    # ── 利润表 ── csmar.cfi_fi2（Income Statement）
    income_sql = f"""
        SELECT tyear, b001000000, b001100000, b002000000, b002001000,
               b002002000, b002003000, b002004000, b003000000, b004000000,
               b004001000, b005000000, b006000000, b008000000
        FROM csmar.cfi_fi2
    ...
    # ── 资产负债表 ── csmar.cfi_fi1（Balance Sheet）
    balance_sql = f"""
        SELECT tyear, a001101000, a001110000, a001121000, a001123000,
               a001200000, a001000000, a002110000, a002111100, a002130000,
               a002140000, a002000000, a000000000,
               b001101000, b001110000, b001120000, b001130000,
    ...
    # ── 公司名称 ──
    name_sql = f"""
        SELECT stknme FROM csmar.stk_listedcoinfannu
        WHERE stkcd = '{ticker_6}'
        LIMIT 1
    """
